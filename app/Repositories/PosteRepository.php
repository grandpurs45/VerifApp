...
final class PosteRepository
{
    ...
    public function findByTypeIdDetailed(int $typeVehiculeId, ?int $caserneId = null): array
    {
        $connection = Database::getConnection();
        ...
        $sql = '
            ...
            FROM postes p
            INNER JOIN type_vehicules tv ON tv.id = p.type_vehicule_id
            WHERE p.type_vehicule_id = :type_vehicule_id
              ' . ($caserneId !== null ? 'AND p.caserne_id = :caserne_id' : '') . '
            ORDER BY p.nom ASC
        ';

        $statement = $connection->prepare($sql);
        $params = ['type_vehicule_id' => $typeVehiculeId];
        if ($caserneId !== null) {
            $params['caserne_id'] = $caserneId;
        }
        $statement->execute($params);
